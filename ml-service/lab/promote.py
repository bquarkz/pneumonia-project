...
Copies the ONNX export and fills in app/model/manifest.yaml with the commit, dataset
version, and the test-set metrics recomputed from lab/artifacts/predictions/
phase4_test_predictions.csv (the same file 05_model_comparison.ipynb reads) — so the
manifest always reflects the predictions actually saved on disk, not numbers copy-pasted
from a notebook's printed output.
...
def main():
    ...
    print(f"  git add -f {onnx_rel} {manifest_rel}")
    print('  git commit -m "..."')
