...
PLACEHOLDER_VERSION = "placeholder-0.0.0"
...
def _placeholder_predict(image_bytes: bytes) -> tuple[bool, float]:
...
class Predictor:
    def __init__(self):
        ...
        if self.model_version and _ONNX_PATH.exists():
            ...
            # loudly, not silently degrade to placeholder predictions.
            ...
            self.model_version = PLACEHOLDER_VERSION
            logger.warning(
                "No promoted model (manifest.model_version or %s missing) — serving placeholder predictions",
                _ONNX_PATH,
            )

    def predict(self, image_bytes: bytes) -> tuple[bool, float]:
        if self._session is None:
            return _placeholder_predict(image_bytes)
